...
function vbsScript(useaiPath: string): string {
  // In a VBScript string literal embedded double-quotes are escaped by
  // doubling them ("" inside a "...."). Wrapping the exe path in quotes
  // handles paths with spaces (e.g. C:\Program Files\nodejs\useai.cmd)
  // correctly when CreateProcess parses the command line.
  const inner = `""${useaiPath}"" daemon-run`;
  return [
    ...
    `CreateObject("WScript.Shell").Run "${inner}", 0, False`,
    ``,
  ].join("\r\n");
}

function installWindows(): void {
  const useaiPath = resolveUseaiPathWin();
  const target = vbsPathWin();
  mkdirSync(dirname(target), { recursive: true });
  writeFileSync(target, vbsScript(useaiPath), "utf-8");
