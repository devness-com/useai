...
const startCommand = new Command('start')
  ...
  .action(async (opts) => {
    ...
    if (pid && isProcessRunning(pid.pid)) {
      ...
      if (health) {
        console.log(pc.yellow(`  Daemon already running (PID ${pid.pid}, port ${pid.port})`));
        return;
      }
    }
    ...
    if (started) {
      ...
      console.log(pc.green(`  ✓ Daemon started (PID ${newPid?.pid ?? 'unknown'}, port ${port})`));
    } else {
      console.log(pc.red(`  ✗ Daemon failed to start within 8 seconds`));
      console.log(pc.dim(`    Try: useai daemon start --foreground`));
    }
  });
...
const stopCommand = new Command('stop')
  ...
  .action(async (opts) => {
    ...
    if (pid && isProcessRunning(pid.pid)) {
      console.log(pc.dim(`  Stopping daemon (PID ${pid.pid})...`));
      await killDaemon();
    ...
    if (pids.length > 0) {
      console.log(pc.dim(`  Stopping daemon on port ${port} (PIDs: ${pids.join(', ')})...`));
      await killDaemon();
      console.log(pc.green(`  ✓ Daemon stopped`));
      return;
    }
...
const statusCommand = new Command('status')
  ...
  .action(async () => {
    ...
    if (!isProcessRunning(pid.pid)) {
      console.log(pc.dim('  Daemon is not running (stale PID file)'));
      return;
    }
    ...
    if (!health) {
      console.log(pc.yellow(`  Daemon process exists (PID ${pid.pid}) but health check failed`));
      return;
    }
    ...
    const autostartStatus = isAutostartInstalled()
      ? pc.green('installed') + pc.dim(` (${platform})`)
...
const autostartInstallCommand = new Command('install')
  ...
  .action(() => {
    ...
    if (platform === 'unsupported') {
      console.log(pc.red(`  ✗ Auto-start is not supported on ${process.platform}`));
      return;
    }

    try {
      installAutostart();
      console.log(pc.green(`  ✓ Auto-start installed (${platform})`));
    } catch (e) {
      console.log(pc.red(`  ✗ Failed to install auto-start: ${(e as Error).message}`));
    }
  });
...
const autostartRemoveCommand = new Command('remove')
  ...
  .action(() => {
    try {
      removeAutostart();
      console.log(pc.green(`  ✓ Auto-start removed`));
    } catch (e) {
      console.log(pc.red(`  ✗ Failed to remove auto-start: ${(e as Error).message}`));
    }
  });
...
const autostartStatusCommand = new Command('status')
  ...
  .action(() => {
    ...
    if (installed) {
      console.log(pc.green(`  ✓ Auto-start is installed (${platform})`));
    } else {
      console.log(pc.dim(`  Auto-start is not installed`));
    }
  });
...
const autostartRecoverCommand = new Command('recover')
  ...
  .action(() => {
    ...
    if (!isAutostartInstalled()) {
      console.log(pc.yellow('  Auto-start is not installed. Run: useai daemon autostart install'));
      return;
    }
